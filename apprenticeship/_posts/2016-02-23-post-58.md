...
The Global Head of Gig Management doesn't really care about the bands but only wants to make sure that the marketing is done right $$$.

In this case we can use the factory method pattern to create a new instance of a gig for a band that will play at a specific venue. The factory method will be declared as abstract in the base class and can then be implemented by any classes that subclass it (in this case the Kentish Town Forum). 
...
public abstract class MobileNetworkVenueGig {
    ...
    public void doMarketing() {
        Band band;
        band = getBand("The Kills");
        printFlyers(band);
        updateWebsite(band);
        tellTicketVendors(band);
    }

    protected abstract Band getBand(String bandName);
...
And now this class can be extended by classes for gigs at a specific gig venue, like the Kentish Town Forum. These venue specific classes inherit all the methods from the base class and implement the abstract method `getBand(String bandName)` themselves. That way, the Global Head of Gig Management can ensure that all gigs are set up following the same procedure.

<pre><code class="language-java">
public class KentishTownForumGig extends MobileNetworkVenueGig {

    @Override
    protected Band getBand(String bandName) {
